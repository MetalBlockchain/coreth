...
function test_import_testing_only_in_tests {
  ...
  IMPORT_FROM_TESTS=$(echo "${NON_TEST_GO_FILES}" | xargs grep -l '"github.com/MetalBlockchain/coreth/tests/')
  IMPORT_TEST_PKG=$(echo "${NON_TEST_GO_FILES}" | xargs grep -lP '"github.com/MetalBlockchain/coreth/.*?test"')
