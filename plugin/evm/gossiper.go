...
type pushGossiper struct {
	...
	recentAtomicTxs *cache.LRU[ids.ID, *Tx]
	recentEthTxs    *cache.LRU[common.Hash, *Tx]
...
func (vm *VM) createGossiper(stats GossipStats) Gossiper {
	...
	net := &pushGossiper{
		...
		recentAtomicTxs:      &cache.LRU[ids.ID, *Tx]{Size: recentCacheSize},
		recentEthTxs:         &cache.LRU[common.Hash, *Tx]{Size: recentCacheSize},
